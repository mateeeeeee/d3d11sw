...
static void MockCS(const SW_CSInput* input, const SW_Resources* res)
{
    ++gInvocationCount;
    gLastInput = *input;
}
...
TEST_F(DispatchExecutorTests, GroupID)
{
    ...
    auto fn = [](const SW_CSInput* input, const SW_Resources*) {
        (void)input;
    };
    ...
    auto captureFn = [](const SW_CSInput* input, const SW_Resources*) {
