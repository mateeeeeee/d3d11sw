...
TEST_F(CSGoldenTests, SampleCmp8x8)
{
    // 8x8 R32_FLOAT depth texture: column x has value x/8.0
    float texData[8 * 8];
    for (int y = 0; y < 8; ++y)
    {
        for (int x = 0; x < 8; ++x)
        {
            texData[y * 8 + x] = (float)x / 8.0f;
        }
    }
    ...
    auto bytecode = ReadBytecode(D3D11SW_SHADER_DIR "/cs_sample_cmp.o");
    ...
    auto result = CheckGolden("cs_sample_cmp_8x8", pixels, 8, 8, 0.01f);
