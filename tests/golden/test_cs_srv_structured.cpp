...
TEST_F(CSGoldenTests, SrvStructured8x8)
{
    const UINT N = 64;
    ...
    D3D11_BUFFER_DESC srvBufDesc{};
    srvBufDesc.ByteWidth           = N * 4;
    srvBufDesc.Usage               = D3D11_USAGE_DEFAULT;
    srvBufDesc.BindFlags           = D3D11_BIND_SHADER_RESOURCE;
    srvBufDesc.MiscFlags           = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;
    srvBufDesc.StructureByteStride = 4;

    D3D11_SUBRESOURCE_DATA srvInit{};
    srvInit.pSysMem = initData.data();

    ID3D11Buffer* srvBuf = nullptr;
    ASSERT_TRUE(SUCCEEDED(device->CreateBuffer(&srvBufDesc, &srvInit, &srvBuf)));
    ...
    ASSERT_TRUE(SUCCEEDED(device->CreateShaderResourceView(srvBuf, &srvDesc, &srv)));
    ...
    auto bytecode = ReadBytecode(D3D11SW_SHADER_DIR "/cs_srv_structured.o");
    ...
    auto result = CheckGolden("cs_srv_structured_8x8", pixels, 8, 8, 0.01f);
