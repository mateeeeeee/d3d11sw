...
TEST_F(DrawGoldenTests, GradientTriangle64x64)
{
    ...
    auto result = CheckGolden("draw_gradient_triangle_64x64", pixels.data(), W, H, 0.02f);
    EXPECT_TRUE(result.passed) << result.message;

    // Sanity: top area should be reddish, bottom-left greenish, bottom-right bluish
    // Check center pixel has a mix of colors
    UINT cx = W / 2, cy = H / 2;
    UINT ci = (cy * W + cx) * 4;
    bool hasMix = (pixels[ci+0] > 0.05f && pixels[ci+1] > 0.05f && pixels[ci+2] > 0.05f);
    EXPECT_TRUE(hasMix) << "Center pixel should have a mix of RGB from interpolation";
