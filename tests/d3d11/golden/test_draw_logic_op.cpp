...
struct DrawGoldenTests : ::testing::Test
{
    ...
    void SetUp() override
    {
        ...
        ASSERT_TRUE(SUCCEEDED(device->QueryInterface(__uuidof(ID3D11Device1), (void**)&device1)));
    }

    void TearDown() override
    {
        if (context) { context->Release(); context = nullptr; }
        if (device1) { device1->Release(); device1 = nullptr; }
        if (device)  { device->Release();  device  = nullptr; }
    }
};

TEST_F(DrawGoldenTests, LogicOpXor)
{
    ...
    blendDesc.RenderTarget[0].LogicOp              = D3D11_LOGIC_OP_XOR;
    blendDesc.RenderTarget[0].BlendEnable          = FALSE;
    blendDesc.RenderTarget[0].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_ALL;

    ID3D11BlendState1* blendState = nullptr;
    ASSERT_TRUE(SUCCEEDED(device1->CreateBlendState1(&blendDesc, &blendState)));
    ...
    context->OMSetRenderTargets(1, reinterpret_cast<ID3D11RenderTargetView**>(&rtv), nullptr);
    ...
    auto result = CheckGolden("draw_logic_op_xor_64x64", pixels.data(), W, H, 0.0f);
    EXPECT_TRUE(result.passed) << result.message;

    int magentaPx = 0;
    for (UINT i = 0; i < W * H; ++i)
    {
        if (pixels[i*4+0] > 0.9f && pixels[i*4+1] < 0.1f && pixels[i*4+2] > 0.9f)
        {
            ++magentaPx;
        }
    }
    EXPECT_GT(magentaPx, 100) << "Expected magenta (XOR of red+blue) pixels in triangle area";

    staging->Release();
    blendState->Release();
    ps->Release(); vs->Release(); il->Release();
    cb->Release(); vb->Release();
    rtv->Release(); rtTex->Release();
}

TEST_F(DrawGoldenTests, LogicOpAnd)
{
    const UINT W = 64, H = 64;
    ...
    D3D11_BLEND_DESC1 blendDesc{};
    blendDesc.RenderTarget[0].LogicOpEnable        = TRUE;
    blendDesc.RenderTarget[0].LogicOp              = D3D11_LOGIC_OP_AND;
    blendDesc.RenderTarget[0].BlendEnable          = FALSE;
    blendDesc.RenderTarget[0].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_ALL;
    ...
    auto result = CheckGolden("draw_logic_op_and_64x64", pixels.data(), W, H, 0.0f);
    EXPECT_TRUE(result.passed) << result.message;
    ...
    EXPECT_GT(redPx, 100) << "Expected red pixels (AND of red+white) in triangle area";
