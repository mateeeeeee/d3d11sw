...
TEST_F(DrawGoldenTests, DepthBias64x64)
{
    ...
    // Two identical triangles at z=0.5.
    // Without bias, the second draw fails LESS (0.5 is not < 0.5).
    // With negative DepthBias, the second triangle's depth is pushed closer,
    // so it passes and overwrites the first.
    float vertices[] = {
        // Triangle 1 (red)
         0.0f,  0.8f, 0.5f,
         0.8f, -0.8f, 0.5f,
    ...
    // Draw 1: red triangle, no bias (default rasterizer state)
    context->PSSetConstantBuffers(0, 1, reinterpret_cast<ID3D11Buffer**>(&cbRed));
    context->Draw(3, 0);

    // Draw 2: green triangle with negative DepthBias — pushes depth closer.
    // For D32_FLOAT: bias = DepthBias * 2^(exp(maxZ)-23).
    // maxZ=0.5 → exp=-1, so r = 2^(-1-23) = 2^-24 ≈ 5.96e-8.
    // DepthBias = -100000 → bias ≈ -0.00596, enough to pass LESS vs 0.5.
    ...
    auto result = CheckGolden("draw_depth_bias_64x64", pixels.data(), W, H, 0.0f);
    EXPECT_TRUE(result.passed) << result.message;

    // The green triangle should have fully overwritten the red one
    // (same geometry, bias pushed it closer → passes depth test)
    ...
    EXPECT_EQ(redPx, 0) << "No red pixels expected — green should fully overwrite";
    EXPECT_GT(greenPx, 500) << "Expected many green pixels from biased triangle";
