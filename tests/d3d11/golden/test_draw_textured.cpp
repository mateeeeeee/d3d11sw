...
TEST_F(DrawGoldenTests, TexturedTriangle64x64)
{
    ...
    // 4x4 checkerboard texture: alternating red and green
    ...
    auto vsBytecode = ReadBytecode(D3DSW_SHADER_DIR "/vs_texcoord.o");
    ASSERT_FALSE(vsBytecode.empty()) << "vs_texcoord.o not found — run compile.sh";
    ...
    auto psBytecode = ReadBytecode(D3DSW_SHADER_DIR "/ps_textured.o");
    ASSERT_FALSE(psBytecode.empty()) << "ps_textured.o not found — run compile.sh";
    ...
    auto result = CheckGolden("draw_textured_triangle_64x64", pixels.data(), W, H, 0.0f);
    ...
    EXPECT_GT(redPx, 50) << "Should have red texels from checkerboard";
    EXPECT_GT(greenPx, 50) << "Should have green texels from checkerboard";
