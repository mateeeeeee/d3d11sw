...
TEST_F(DrawGoldenTests, DualSourceBlend)
{
    ...
    struct { float mainColor[4]; float src1Color[4]; } cbData = {
        {1.f, 0.f, 0.f, 1.f},
        {0.f, 0.f, 0.f, 0.5f},
    };
    ...
    auto psBytecode = ReadBytecode(D3DSW_SHADER_DIR "/ps_dual_source.o");
    ...
    blendDesc.RenderTarget[0].DestBlend             = D3D11_BLEND_SRC1_ALPHA;
    ...
    auto result = CheckGolden("draw_dual_source_64x64", pixels.data(), W, H, 0.0f);
    ...
    for (UINT i = 0; i < W * H; ++i)
    {
        ...
        if (r > 0.9f && b > 0.3f && b < 0.7f)
        {
            foundBlended = true;
            break;
        }
    }
    EXPECT_TRUE(foundBlended) << "Expected blended pixels: red(1)*1 + blue(1)*0.5 = (1,0,0.5)";
