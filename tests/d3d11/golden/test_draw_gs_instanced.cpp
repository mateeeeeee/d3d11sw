...
TEST_F(DrawGoldenTests, GSInstanced64x64)
{
    ...
    float vertices[] = {
         0.0f,  0.7f, 0.5f,   1.f, 1.f, 1.f, 1.f,
         0.6f, -0.5f, 0.5f,   1.f, 1.f, 1.f, 1.f,
        -0.6f, -0.5f, 0.5f,   1.f, 1.f, 1.f, 1.f,
    };
    ...
    auto gsBytecode = ReadBytecode(D3DSW_SHADER_DIR "/gs_instanced.o");
    ...
    auto result = CheckGolden("draw_gs_instanced_64x64", pixels.data(), W, H, 0.0f);
    ...
    for (UINT i = 0; i < W * H; ++i)
    {
        ...
        if (g > 0.8f && r < 0.5f && b < 0.5f) { ++greenPx; }
        if (b > 0.8f && r < 0.5f && g < 0.5f) { ++bluePx; }
    }
    EXPECT_GT(redPx, 20)   << "Expected red-tinted triangle (instance 0)";
    EXPECT_GT(greenPx, 20) << "Expected green-tinted triangle (instance 1)";
    EXPECT_GT(bluePx, 20)  << "Expected blue-tinted triangle (instance 2)";
