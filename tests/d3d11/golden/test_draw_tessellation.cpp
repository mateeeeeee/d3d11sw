...
TEST_F(DrawGoldenTests, TessTriSimple)
{
    ...
    auto vsBC = ReadBytecode(D3DSW_SHADER_DIR "/vs_passthrough.o");
    ...
    auto hsBC = ReadBytecode(D3DSW_SHADER_DIR "/hs_simple_tri.o");
    ...
    auto dsBC = ReadBytecode(D3DSW_SHADER_DIR "/ds_simple_tri.o");
    ...
    auto psBC = ReadBytecode(D3DSW_SHADER_DIR "/ps_solid_red.o");
...
TEST_F(DrawGoldenTests, TessQuadSimple)
{
    ...
    auto hsBC = ReadBytecode(D3DSW_SHADER_DIR "/hs_simple_quad.o");
    ASSERT_FALSE(hsBC.empty()) << "HS not compiled — run compile.sh";
    ID3D11HullShader* hs = nullptr;
    ASSERT_TRUE(SUCCEEDED(device->CreateHullShader(hsBC.data(), hsBC.size(), nullptr, &hs)));
    context->HSSetShader(hs, nullptr, 0);

    auto dsBC = ReadBytecode(D3DSW_SHADER_DIR "/ds_simple_quad.o");
    ASSERT_FALSE(dsBC.empty()) << "DS not compiled — run compile.sh";
    ID3D11DomainShader* ds = nullptr;
    ASSERT_TRUE(SUCCEEDED(device->CreateDomainShader(dsBC.data(), dsBC.size(), nullptr, &ds)));
    context->DSSetShader(ds, nullptr, 0);
...
TEST_F(DrawGoldenTests, TessTriColor)
{
    ...
    auto vsBC = ReadBytecode(D3DSW_SHADER_DIR "/vs_color.o");
    ASSERT_FALSE(vsBC.empty());
    ...
    auto hsBC = ReadBytecode(D3DSW_SHADER_DIR "/hs_color_tri.o");
    ASSERT_FALSE(hsBC.empty()) << "HS not compiled — run compile.sh";
    ID3D11HullShader* hs = nullptr;
    ASSERT_TRUE(SUCCEEDED(device->CreateHullShader(hsBC.data(), hsBC.size(), nullptr, &hs)));
    context->HSSetShader(hs, nullptr, 0);

    auto dsBC = ReadBytecode(D3DSW_SHADER_DIR "/ds_color_tri.o");
    ASSERT_FALSE(dsBC.empty()) << "DS not compiled — run compile.sh";
    ID3D11DomainShader* ds = nullptr;
    ASSERT_TRUE(SUCCEEDED(device->CreateDomainShader(dsBC.data(), dsBC.size(), nullptr, &ds)));
    context->DSSetShader(ds, nullptr, 0);

    auto psBC = ReadBytecode(D3DSW_SHADER_DIR "/ps_color.o");
    ID3D11PixelShader* ps = nullptr; device->CreatePixelShader(psBC.data(), psBC.size(), nullptr, &ps);
    context->PSSetShader(ps, nullptr, 0);
...
TEST_F(DrawGoldenTests, TessTriColorGS)
{
    ...
    auto gsBC = ReadBytecode(D3DSW_SHADER_DIR "/gs_explode.o");
...
TEST_F(DrawGoldenTests, TessTriSVPosition)
{
    ...
    auto hsBC = ReadBytecode(D3DSW_SHADER_DIR "/hs_svpos_tri.o");
    ASSERT_FALSE(hsBC.empty()) << "HS not compiled — run compile.sh";
    ID3D11HullShader* hs = nullptr;
    ASSERT_TRUE(SUCCEEDED(device->CreateHullShader(hsBC.data(), hsBC.size(), nullptr, &hs)));
    context->HSSetShader(hs, nullptr, 0);

    auto dsBC = ReadBytecode(D3DSW_SHADER_DIR "/ds_svpos_tri.o");
    ASSERT_FALSE(dsBC.empty()) << "DS not compiled — run compile.sh";
    ID3D11DomainShader* ds = nullptr;
    ASSERT_TRUE(SUCCEEDED(device->CreateDomainShader(dsBC.data(), dsBC.size(), nullptr, &ds)));
    context->DSSetShader(ds, nullptr, 0);
    ...
    auto px = Readback(device, context, rtTex, W, H);
    auto result = CheckGolden("draw_tess_tri_svpos", px.data(), W, H, 0.0f);
    EXPECT_TRUE(result.passed) << result.message;
    ...
    EXPECT_GT(redPixels, 500) << "Expected tessellated triangle to produce red pixels"
        " (SV_Position must be mirrored to .o[reg] in HS CP epilogue)";
