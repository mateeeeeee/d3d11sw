...
TEST_F(DrawGoldenTests, TriangleStrip64x64)
{
    ...
    auto result = CheckGolden("draw_tristrip_64x64", pixels.data(), W, H, 0.0f);
    EXPECT_TRUE(result.passed) << result.message;

    int redPixels = 0;
    for (UINT i = 0; i < W * H; ++i)
    {
        if (pixels[i * 4 + 0] > 0.9f && pixels[i * 4 + 1] < 0.1f) { ++redPixels; }
    }
    EXPECT_GT(redPixels, 2000) << "Triangle strip quad should fill most of the 64x64 area";
...
TEST_F(DrawGoldenTests, LineList64x64)
{
    const UINT W = 64, H = 64;
    ...
    auto result = CheckGolden("draw_linelist_64x64", pixels.data(), W, H, 0.0f);
    EXPECT_TRUE(result.passed) << result.message;
    ...
    EXPECT_GT(redPixels, 50) << "Two diagonal lines should produce at least 50 red pixels";
...
TEST_F(DrawGoldenTests, PointList64x64)
{
    const UINT W = 64, H = 64;
    ...
    auto result = CheckGolden("draw_pointlist_64x64", pixels.data(), W, H, 0.0f);
    EXPECT_TRUE(result.passed) << result.message;
    ...
    EXPECT_EQ(redPixels, 5) << "5 points should produce exactly 5 red pixels";
