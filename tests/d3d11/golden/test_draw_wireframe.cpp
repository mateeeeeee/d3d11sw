...
TEST_F(DrawGoldenTests, Wireframe64x64)
{
    ...
    auto result = CheckGolden("draw_wireframe_64x64", pixels.data(), W, H, 0.0f);
    ...
    EXPECT_GT(redPx, 30) << "Wireframe edges should produce visible red pixels";

    UINT cx = W / 2, cy = H / 2 + 4;
    UINT ci = (cy * W + cx) * 4;
    EXPECT_LT(pixels[ci + 0], 0.1f) << "Center pixel should be clear (not filled) in wireframe mode";
