...
TEST_F(DrawGoldenTests, ScissorClip64x64)
{
    ...
    // Scissor rect: only allow the top-left 32x32 quadrant
    D3D11_RECT scissor = {0, 0, 32, 32};
    ...
    auto result = CheckGolden("draw_scissor_64x64", pixels.data(), W, H, 0.0f);
    EXPECT_TRUE(result.passed) << result.message;

    // Verify: no red pixels outside the scissor rect (x>=32 or y>=32)
    bool outsideLeak = false;
    for (UINT y = 0; y < H; ++y)
    {
        for (UINT x = 0; x < W; ++x)
        {
    ...
    EXPECT_FALSE(outsideLeak) << "Red pixels found outside scissor rect";
