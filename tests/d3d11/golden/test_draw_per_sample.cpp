...
TEST_F(DrawGoldenTests, PerSampleShading64x64)
{
    ...
    auto psBytecode = ReadBytecode(D3DSW_SHADER_DIR "/ps_per_sample.o");
    ASSERT_FALSE(psBytecode.empty()) << "ps_per_sample.o not found — run compile.sh";
    ...
    context->ResolveSubresource(resolveTex, 0, msTex, 0, DXGI_FORMAT_R8G8B8A8_UNORM);
    ...
    auto result = CheckGolden("draw_per_sample_shading_64x64", pixels.data(), W, H, 0.02f);
    ...
    for (UINT i = 0; i < W * H; ++i)
    {
        ...
        if (r > 0.4f && r < 0.6f && g > 0.4f && g < 0.6f && b < 0.1f)
        {
            ++yellowPixels;
        }
    }
    EXPECT_GT(yellowPixels, W * H / 2)
        << "Expected most pixels to be yellow (~0.5, 0.5, 0) from per-sample shading averaging, got " << yellowPixels;
