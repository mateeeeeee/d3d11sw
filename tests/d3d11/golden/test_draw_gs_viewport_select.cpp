...
TEST_F(DrawGoldenTests, GSViewportSelect64x64)
{
    ...
    auto gsBytecode = ReadBytecode(D3DSW_SHADER_DIR "/gs_viewport_select.o");
    ...
    viewports[0].Width    = static_cast<FLOAT>(W / 2);
    viewports[0].Height   = static_cast<FLOAT>(H);
    viewports[0].MinDepth = 0.f;
    viewports[0].MaxDepth = 1.f;
    viewports[1].TopLeftX = static_cast<FLOAT>(W / 2);
    viewports[1].TopLeftY = 0.f;
    viewports[1].Width    = static_cast<FLOAT>(W / 2);
    viewports[1].Height   = static_cast<FLOAT>(H);
    viewports[1].MinDepth = 0.f;
    viewports[1].MaxDepth = 1.f;
    ...
    auto result = CheckGolden("draw_gs_viewport_select_64x64", pixels.data(), W, H, 0.0f);
    EXPECT_TRUE(result.passed) << result.message;

    int leftRedPx = 0, rightBluePx = 0;
    for (UINT y = 0; y < H; ++y)
    {
        for (UINT x = 0; x < W; ++x)
        {
            ...
            if (x < W / 2 && r > 0.8f && g < 0.5f && b < 0.5f) { ++leftRedPx; }
            if (x >= W / 2 && b > 0.8f && r < 0.5f && g < 0.5f) { ++rightBluePx; }
        }
    }
    EXPECT_GT(leftRedPx, 50)   << "Expected red triangle in left viewport";
    EXPECT_GT(rightBluePx, 50) << "Expected blue triangle in right viewport";
