...
static D3DSW_ParsedShader MakeMinimalVS()
{
    D3DSW_ParsedShader s{};
    s.type     = D3DSW_ShaderType::Vertex;
    s.numTemps = 1;

    D3DSW_ShaderSignatureElement svPos{};
...
static D3DSW_ParsedShader MakeCSShader(std::vector<SM4Instruction> instrs,
                                          Uint32 numTemps = 2)
{
    D3DSW_ParsedShader s{};
    s.type     = D3DSW_ShaderType::Compute;
    s.numTemps = numTemps;
...
{
    std::string src = EmitShader(MakeMinimalVS(), "core/shaders/shader_runtime.h");
...
{
    D3DSW_ParsedShader s{};
    s.type     = D3DSW_ShaderType::Pixel;
    s.numTemps = 1;
    s.instrs.push_back(MakeInstr(D3D10_SB_OPCODE_RET));

    std::string src = EmitShader(s, "core/shaders/shader_runtime.h");
...
{
    std::string src = EmitShader(MakeMinimalVS(), "core/shaders/shader_runtime.h");
    EXPECT_NE(src.find("#include \"core/shaders/shader_runtime.h\""), std::string::npos);
}

TEST_F(ShaderCodeGenTests, TempArraySize)
{
    D3DSW_ParsedShader s = MakeMinimalVS();
    s.numTemps = 4;
