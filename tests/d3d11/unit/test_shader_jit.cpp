...
{
    EXPECT_EQ(jit.GetOrCompile(nullptr, 0, D3DSW_ShaderType::Vertex, &DXBCParse, D3DSW_ShaderFrontend::D3D11), nullptr);
}

TEST_F(ShaderJITTests, InvalidMagicReturnsNull)
{
    Uint8 garbage[64];
    EXPECT_EQ(jit.GetOrCompile(garbage, sizeof(garbage), D3DSW_ShaderType::Vertex, &DXBCParse, D3DSW_ShaderFrontend::D3D11), nullptr);
}

TEST_F(ShaderJITTests, ValidVSReturnsNonNull)
{
    auto blob = BuildDXBCBlob(FOURCC_SHDR, DwordsToBytes(MakeMinimalVSTokens()));
    void* fn = jit.GetOrCompile(blob.data(), blob.size(), D3DSW_ShaderType::Vertex, &DXBCParse, D3DSW_ShaderFrontend::D3D11);
...
{
    auto blob = BuildDXBCBlob(FOURCC_SHDR, DwordsToBytes(MakeMinimalVSTokens()));
    void* fn1 = jit.GetOrCompile(blob.data(), blob.size(), D3DSW_ShaderType::Vertex, &DXBCParse, D3DSW_ShaderFrontend::D3D11);
    void* fn2 = jit.GetOrCompile(blob.data(), blob.size(), D3DSW_ShaderType::Vertex, &DXBCParse, D3DSW_ShaderFrontend::D3D11);
...
TEST_F(ShaderJITTests, DifferentBytecodeDifferentPointer)
{
    ...
    void* fn1 = jit.GetOrCompile(blob1.data(), blob1.size(), D3DSW_ShaderType::Vertex, &DXBCParse, D3DSW_ShaderFrontend::D3D11);
    void* fn2 = jit.GetOrCompile(blob2.data(), blob2.size(), D3DSW_ShaderType::Vertex, &DXBCParse, D3DSW_ShaderFrontend::D3D11);
    ASSERT_NE(fn1, nullptr);
