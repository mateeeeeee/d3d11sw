#include "d3d11/shaders/dxbc_parser.h"
#include "d3d11/shaders/sm4_decoder.h"
...
static Bool ParseSignatureChunk(const Uint8* data, Usize size,
                                std::vector<D3DSW_ShaderSignatureElement>& out,
                                Bool osg5 = false)
{
    if (size < 8)
    {
        D3DSW_ERROR("ParseSignatureChunk: chunk too small ({} < 8)", size);
        return false;
    }
...
static D3DSW_ShaderType ShaderTypeFromRdef(Uint16 shaderType)
{
    switch (shaderType)
    {
        case RDEF_SHTYPE_PS: return D3DSW_ShaderType::Pixel;
        case RDEF_SHTYPE_VS: return D3DSW_ShaderType::Vertex;
        case RDEF_SHTYPE_GS: return D3DSW_ShaderType::Geometry;
        case RDEF_SHTYPE_HS: return D3DSW_ShaderType::Hull;
        case RDEF_SHTYPE_DS: return D3DSW_ShaderType::Domain;
        case RDEF_SHTYPE_CS: return D3DSW_ShaderType::Compute;
        default:             return D3DSW_ShaderType::Unknown;
    }
}

static Bool ParseRdefChunk(const Uint8* data, Usize size, D3DSW_ParsedShader& out)
{
    if (size < sizeof(DXBCRdefHeader))
    {
        D3DSW_ERROR("ParseRdefChunk: chunk too small ({} < {})", size, sizeof(DXBCRdefHeader));
        return false;
    }
...
static Bool ParseShaderChunk(const Uint8* data, Usize size, D3DSW_ParsedShader& out)
{
    if (size < 8)
    {
        D3DSW_ERROR("ParseShaderChunk: chunk too small ({} < 8)", size);
        return false;
    }
    ...
    {
        D3DSW_ERROR("ParseShaderChunk: SM4Decode failed");
        return false;
    }
    ...
    for (const auto& instr : out.instrs)
    {
        ...
        if (out.type == D3DSW_ShaderType::Pixel &&
...
Bool DXBCParseReflection(const void* bytecode, Usize len, D3DSW_ParsedShader& out)
{
    if (!bytecode || len < sizeof(DXBCContainerHeader) + 4)
    {
        D3DSW_ERROR("DXBCParseReflection: invalid bytecode (null={}, len={})", !bytecode, len);
        return false;
    }
    ...
    {
        D3DSW_ERROR("DXBCParseReflection: bad magic 0x{:08X}", hdr.magic);
        return false;
    }
    if (hdr.version != 1)
    {
        D3DSW_ERROR("DXBCParseReflection: unsupported version {}", hdr.version);
        return false;
    }
    if (hdr.totalSize > len)
    {
        D3DSW_ERROR("DXBCParseReflection: totalSize {} > buffer len {}", hdr.totalSize, len);
        return false;
    }

    const Uint32* offsets = reinterpret_cast<const Uint32*>(base + sizeof(DXBCContainerHeader));
    out = {};
    out.csThreadGroupX = out.csThreadGroupY = out.csThreadGroupZ = 1;
...
Bool DXBCParse(const void* bytecode, Usize len, D3DSW_ParsedShader& out)
{
    if (!DXBCParseReflection(bytecode, len, out))
    {
        D3DSW_ERROR("DXBCParse: DXBCParseReflection failed");
        return false;
    }
