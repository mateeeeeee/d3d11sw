...
Bool SM4Decode(const Uint32* tokens, Uint32 numDwords, D3DSW_ParsedShader& out)
{
    out.numTemps = 0;
    ...
    if (numDwords < 2)
    {
        D3DSW_ERROR("SM4Decode: bytecode too short ({} dwords)", numDwords);
        return false;
    }
    ...
    std::vector<D3DSW_IndexableTempDecl>* activeIdxTemps = &out.indexableTemps;
    ...
    while (pos < totalLen)
    {
        ...
        if (op == D3D11_SB_OPCODE_DCL_THREAD_GROUP)
        {
            if (pos + 3 < instrEnd)
            {
                out.csThreadGroupX = tokens[pos + 1];
                out.csThreadGroupY = tokens[pos + 2];
                out.csThreadGroupZ = tokens[pos + 3];
            }
            pos = instrEnd;
            continue;
        }
