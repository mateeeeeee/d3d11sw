...
void D3D11HullShaderSW::Compile()
{
    ...
    GetShaderJIT().GetOrCompile(_bytecode.data(), _bytecode.size(), D3DSW_ShaderType::Hull, &DXBCParse, D3DSW_ShaderFrontend::D3D11);

    if (!_reflection.hsControlPointPhase.instrs.empty())
    {
        _cpFn = reinterpret_cast<SW_HSCPFn>(
            GetShaderJIT().FindSymbol(_bytecode.data(), _bytecode.size(), D3DSW_ShaderFrontend::D3D11, "HS_CP"));
    }
    ...
    for (Uint32 i = 0; i < _reflection.hsForkPhases.size(); ++i)
    {
        ...
        _forkFns[i] = reinterpret_cast<SW_HSForkJoinFn>(
            GetShaderJIT().FindSymbol(_bytecode.data(), _bytecode.size(), D3DSW_ShaderFrontend::D3D11, name.c_str()));
    }
