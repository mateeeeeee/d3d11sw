...
Uint8 FindSVPositionReg(const D3DSW_ParsedShader& shader)
{
...
void EmitIndexableTemps(CodeWriter& w, const D3DSW_ParsedShader& shader)
{
...
void EmitIndexableTempsFromList(CodeWriter& w, const std::vector<D3DSW_IndexableTempDecl>& temps)
{
    for (const D3DSW_IndexableTempDecl& it : temps)
    {
        w.Line("SW_float4 x{}[{}] = {{}};", it.regIndex, it.numRegs);
    }
}

Uint32 FindTexDimension(const D3DSW_ParsedShader& shader, Uint32 slot)
{
...
template<D3DSW_ShaderType Type = D3DSW_ShaderType::Vertex>
std::string EmitDstBase(const SM4Operand& op)
{
    constexpr Bool Quad = (Type == D3DSW_ShaderType::Pixel);
    ...
    case D3D11_SB_OPERAND_TYPE_OUTPUT_CONTROL_POINT:
        if constexpr (Type == D3DSW_ShaderType::Hull)
        {
            static const Char* cn[] = {"x","y","z","w"};
            if (op.relativeReg >= 0)
            {
...
std::string EmitSrc(const SM4Operand& op)
{
    ...
    case D3D10_SB_OPERAND_TYPE_INPUT:
        if constexpr (Type == D3DSW_ShaderType::Geometry)
        {
            if (op.indexDim >= 2)
            {
                static const Char* cn[] = {"x","y","z","w"};
        ...
        else if constexpr (Type == D3DSW_ShaderType::Hull)
        {
            if (op.indexDim >= 2)
            {
                static const Char* cn[] = {"x","y","z","w"};
        ...
        else if constexpr (Type == D3DSW_ShaderType::Domain)
        {
...
void EmitGSEmitVertex(CodeWriter& w, const D3DSW_ParsedShader& shader, Uint32 stream = 0)
{
...
void EmitInstr(CodeWriter& w, const SM4Instruction& instr,
               const D3DSW_ParsedShader& shader)
{
    constexpr Bool Quad = (Type == D3DSW_ShaderType::Pixel);
    Bool isIntOp = false;
    switch (instr.op)
    ...
    case D3D10_SB_OPCODE_SAMPLE:
        if (dst && src0 && src1 && src2)
        {
            ...
            if constexpr (Quad)
            {
                Uint32 qdim = FindTexDimension(shader, src1->indices[0]);
                Int su = src0->swizzle[0];
                Int sv = src0->swizzle[1];
                Int sw_ = src0->swizzle[2];
                ...
                {
                    // 2D: compute screen-space derivatives for implicit LOD
                    std::string uvBase;
...
template<D3DSW_ShaderType Type = D3DSW_ShaderType::Vertex>
void EmitInstructions(CodeWriter& w, const D3DSW_ParsedShader& shader)
{
...
template<D3DSW_ShaderType Type>
void EmitInstructionsFromList(CodeWriter& w, const std::vector<SM4Instruction>& instrs, const D3DSW_ParsedShader& shader)
{
...
void EmitInstructionsQuad(CodeWriter& w, const D3DSW_ParsedShader& shader)
{
    for (const auto& instr : shader.instrs)
    {
        if (IsDeriv(instr.op))
        ...
        if (instr.op == D3D10_SB_OPCODE_DISCARD)
        {
            ...
            w.Line("if (sw_bits_uint({}.x) {} 0u) {{ _discarded |= (1u << _q); }}",
                   EmitSrc<D3DSW_ShaderType::Pixel>(instr.operands[0]), instr.testNonZero ? "!=" : "==");
            w.Dedent();
            w.Line("}}}}");
            continue;
        }
        ...
        if (instr.op == D3D10_SB_OPCODE_SWITCH)
        {
            if (!instr.operands.empty())
            {
                w.Line("{{ unsigned _sw_val = sw_bits_uint({}.x);", EmitSrc<D3DSW_ShaderType::Pixel>(instr.operands[0]));
                w.Line("_as[_ai++] = _active;");
            }
            continue;
        }
        ...
        w.Line("for (int _q = 0; _q < 4; ++_q) {{ if (_active & (1u << _q)) {{");
        w.Indent();
        EmitInstr<D3DSW_ShaderType::Pixel>(w, instr, shader);
        w.Dedent();
        w.Line("}}}}");
    }
}

void EmitPS(CodeWriter& w, const D3DSW_ParsedShader& shader)
{
    ...
    w.Line("SW_float4 out_v[{}][4] = {{}};", (Int)SW_MAX_PS_OUTPUTS);
    ...
    w.Indent();
    for (Uint32 i = 0; i < SW_MAX_PS_OUTPUTS; ++i)
    {
...
void EmitVSOutputEpilogue(CodeWriter& w, const D3DSW_ParsedShader& shader)
{
    for (const auto& e : shader.outputs)
    {
        if (e.svType == D3D_NAME_POSITION)
        ...
        else if (std::strcmp(e.name, "FOG") == 0)
        {
            w.Line("out_ptr->fogFactor = out_v[{}].x;", e.reg);
        }
        else
        {
...
void EmitVS(CodeWriter& w, const D3DSW_ParsedShader& shader)
{
    Uint32 numTemps = shader.numTemps > 0 ? shader.numTemps : 1;
    Uint8 svPosReg = FindSVPositionReg(shader);
    ...
    w.Line("SW_float4 out_v[{}] = {{}};", (Int)SW_MAX_VARYINGS);
    w.Newline();

    EmitInstructions<D3DSW_ShaderType::Vertex>(w, shader);
...
void EmitCS(CodeWriter& w, const D3DSW_ParsedShader& shader)
{
    Uint32 numTemps = shader.numTemps > 0 ? shader.numTemps : 1;
    ...
    EmitInstructions<D3DSW_ShaderType::Compute>(w, shader);
...
void EmitGS(CodeWriter& w, const D3DSW_ParsedShader& shader)
{
    Uint32 numTemps = shader.numTemps > 0 ? shader.numTemps : 1;
    ...
    EmitInstructions<D3DSW_ShaderType::Geometry>(w, shader);
...
void EmitDS(CodeWriter& w, const D3DSW_ParsedShader& shader)
{
    Uint32 numTemps = shader.numTemps > 0 ? shader.numTemps : 1;
    ...
    EmitInstructions<D3DSW_ShaderType::Domain>(w, shader);
...
void EmitHS(CodeWriter& w, const D3DSW_ParsedShader& shader)
{
    if (!shader.hsControlPointPhase.instrs.empty())
    {
        ...
        EmitInstructionsFromList<D3DSW_ShaderType::Hull>(w, phase.instrs, shader);
...
std::string EmitShader(const D3DSW_ParsedShader& shader,
                       const std::string& runtimeHeaderPath)
{
    ...
    switch (shader.type)
    {
    case D3DSW_ShaderType::Vertex:  EmitVS(w, shader); break;
    case D3DSW_ShaderType::Pixel:
        EmitPS(w, shader);
        break;
    case D3DSW_ShaderType::Compute: EmitCS(w, shader); break;
    case D3DSW_ShaderType::Geometry: EmitGS(w, shader); break;
    case D3DSW_ShaderType::Hull:   EmitHS(w, shader); break;
    case D3DSW_ShaderType::Domain: EmitDS(w, shader); break;
    default:
        w.Line("extern \"C\" SW_JIT_EXPORT void ShaderMain() {{}}");
        break;
    }
