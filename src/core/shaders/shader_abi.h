...
struct SW_SRV
{
    ...
    // Per-channel swizzle applied after sampling (indices 0-3 = RGBA component,
    // 4 = constant 0.0, 5 = constant 1.0). Only applied when hasSwizzle != 0.
    unsigned char swizzle[4];
    unsigned char hasSwizzle;
};
...
struct SW_VSOutput { SW_float4 pos; SW_float4 o[SW_MAX_VARYINGS]; float clipDist[8]; float cullDist[8]; unsigned viewportIndex = 0; unsigned rtArrayIndex = 0; float fogFactor = 1.f; };
