...
static inline unsigned sw_format_stride(unsigned fmt)
{
    switch (fmt)
    {
        ...
        case SW_FORMAT_B8G8R8X8_UNORM:
        case SW_FORMAT_B8G8R8A8_UNORM_SRGB:
        case SW_FORMAT_B8G8R8X8_UNORM_SRGB:
                                           ...
                                           unsigned x, unsigned y)
{
    ...
    switch (fmt)
    {
        ...
        case SW_FORMAT_B8G8R8X8_UNORM:
            return { p[2]/255.f, p[1]/255.f, p[0]/255.f, 1.f };
        case SW_FORMAT_B8G8R8A8_UNORM_SRGB:
            return { sw_srgb_to_linear(p[2]/255.f), sw_srgb_to_linear(p[1]/255.f), sw_srgb_to_linear(p[0]/255.f), p[3]/255.f };
        case SW_FORMAT_B8G8R8X8_UNORM_SRGB:
            return { sw_srgb_to_linear(p[2]/255.f), sw_srgb_to_linear(p[1]/255.f), sw_srgb_to_linear(p[0]/255.f), 1.f };
        case SW_FORMAT_R8G8B8A8_SNORM:
...
static inline SW_float4 sw_apply_srv_swizzle(const SW_SRV& t, SW_float4 c)
{
    if (!t.hasSwizzle) { return c; }
    const float comps[6] = { c.x, c.y, c.z, c.w, 0.f, 1.f };
    return { comps[t.swizzle[0]], comps[t.swizzle[1]],
             comps[t.swizzle[2]], comps[t.swizzle[3]] };
}
                                      ...
                                      float u, float v)
{
    return sw_apply_srv_swizzle(t,
        sw_sample_2d_at(static_cast<const unsigned char*>(t.data), t.format,
                                      ...
                                      float u, float v, float w)
{
        ...
        (s.filter & 0x7F) == SW_FILTER_MIN_MAG_POINT_MIP_LINEAR)
    {
        ...
        return sw_apply_srv_swizzle(t,
            sw_sample_2d_at(base + (unsigned long long)pz * sp, t.format, tw, th, rp, s, u, v));
    }
                                        ...
                                        float x, float y, float z)
{
    ...
    return sw_apply_srv_swizzle(t,
        sw_sample_2d_at(faceData, t.format, t.mips[0].width, faceH, rp, s, fu, fv));
}
