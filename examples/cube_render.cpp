...
{
    auto* device  = (ID3D11Device*)app.device;
    ...
    float rotY[16], rotX[16], rot[16], view[16], proj[16], vp[16], mvp[16];

    MakeRotationY(time * 0.7f, rotY);
    MakeRotationX(time * 0.3f, rotX);
    MatMul4x4(rotX, rotY, rot);

    MakeLookAt({0.f, 0.f, 2.5f}, {0.f, 0.f, 0.f}, {0.f, 1.f, 0.f}, view);
    ...
    for (int r = 0; r < 4; ++r)
    {
        for (int c = 0; c < 4; ++c)
        {
            mvpT[c * 4 + r] = mvp[r * 4 + c];
        }
    }

    ctx->UpdateSubresource(cb, 0, nullptr, mvpT, 0, 0);
