...
// GCC complains about COM interfaces not having virtual destructors
...
// Calling conventions are no-ops on non-Windows
...
#define DECLARE_INTERFACE(x) \
    typedef interface x { const struct x##Vtbl *lpVtbl; } x; \
    typedef const struct x##Vtbl x##Vtbl; \
    const struct x##Vtbl
#else
#define DECLARE_INTERFACE(x) \
    typedef interface x { struct x##Vtbl *lpVtbl; } x; \
