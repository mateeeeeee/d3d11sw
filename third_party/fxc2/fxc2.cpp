...
int main(int argc, char* argv[])
{
      ...
      case 'F':
        switch(optarg[0])
        {
          case 'h':
            ...
            if(verbose) {
              printf ("option -Fh (Output Header) with arg %s\n", &optarg[1]);
            }
            break;
          case 'o':
            outputFileBinary = strdup(&optarg[1]);
            if(verbose) {
              printf ("option -Fo (Output Binary) with arg %s\n", &optarg[1]);
            }
            break;
          default:
            print_usage_arg();
  ...
  if(outputFile == NULL && outputFileBinary == NULL)
    print_usage_missing("outputFile (-Fh or -Fo)");
  if(outputFile != NULL && variableName == NULL)
  ...
  if (FAILED(hr)) {
   ...
   return hr;
    ...
    if(outputFileBinary != NULL) {
      FILE* f;
      errno_t err = fopen_s(&f, outputFileBinary, "wb");

      fwrite(outString, 1, len, f);
      fclose(f);

      if(verbose) {
        printf("Wrote %i bytes of raw bytecode to %s\n", len, outputFileBinary);
      }
    }
